...
另外的主要问题是校对。如果有已校版本，会省去不少麻烦（也就没这个章节）。在这个.srt文件的末尾，可看到注释：
...
用AMD显卡的用户可参考[这个视频](https://www.youtube.com/watch?v=YaBdOHsg-4I)，设置OBS录制参数：
...
命令提示符（Command Prompt），也就是可执行文件cmd.exe，是Windows系统默认的命令行解释器（Command-line Interpreter）。后者所解释的语境通常是给定的操作系统或编程语言中的一种——这一类连接用户与系统内核、解释和执行指令、返回结果的，通常称为shell（壳层或壳）。

[Cmder](https://cmder.app)，自称是Windows上最好的shell，组合了[Clink](https://chrisant996.github.io/clink/) 和模拟终端[ConEmu](https://conemu.github.io)等，提供命令行界面CLI（Command-Line Interface）、图形用户界面GUI（Graphical User Interface）两者。
...
因为Cmder需要配置后再使用。这里讲应急品CMD的使用方法。
...
![](../media/La.isla.minima.2014.mkv_snapshot_00.03.26.png)
...
![](../media/La.isla.minima.2014.mkv_snapshot_00.03.51.png)
...
![](../media/La.isla.minima.2014.mkv_snapshot_01.07.58.png)
...
![](../media/La.isla.minima.2014.mkv_snapshot_00.16.09.png)
...
字幕的主体「对白」，和画中文字、音响、演出等「注释」，它们的时间轴有重叠，就需要制成.ass（Advanced SubStation Alpha）等格式来保存。

之后涉及到文本编辑器[Sublime Text](https://www.sublimetext.com)、字幕编辑器[Subtitle Edit](https://nikse.dk/SubtitleEdit)、特效字幕编辑器[Aegisub](https://aegi.vmoe.info) 里的操作，这里不赘述。

## thumbnail.jpg

最后，用媒体缩略图工具[mt](https://github.com/mutschler/mt)或[mtn](https://gitlab.com/movie_thumbnailer/mtn/-/wikis/home)来生成一张4x4格的图：
...
![](../media/La.isla.minima.2014.mt.jpg)
...
4. 文本编辑器[Obsidian](https://obsidian.md)：以轻量级标记语言.md（Markdown）文本格式编辑了这篇文，并且用内置的功能，渲染和导出了.pdf（Portable Document Format）文件，作为并未使用的额外的附件。
...
5. Rust包[lychee](https://github.com/lycheeverse/lychee): 排查文本里的链接的情况。
...
1. 使用机器辅助翻译，但不做润色或修饰，保留机翻的风格
2. 对拟声等同类句型采用一致的处理，如同声复读时，只写两组字幕
3. 在作分隔的标点处拆分句子
4. 拆分人物对话，除非对答过快
5. 时间轴的宽度对齐音频
6. 译文句子的语序对齐原文，也就是音频
